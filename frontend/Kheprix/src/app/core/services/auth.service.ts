...
export class AuthService extends BaseService {
  ...
  solicitarRedefinicao(email: string): Observable<{ mensagem: string }> {
    return this.post<{ mensagem: string }>('/autenticacao/solicitar_redefinicao', { email });
  }

  validarTokenRedefinicao(token: string): Observable<{ valido: boolean }> {
    return this.post<{ valido: boolean }>('/autenticacao/validar_token_redefinicao', { token });
  }
