...
import { Estudo, Campanha, UnidadeAmostral, EventoAmostragem } from '../../models';
...
export class CadastroRapidoComponent implements OnInit {
  ...
  onEstudoChange() {
    this.campanhaId = ''; this.unidadeId = ''; this.eventoId = '';
    this.campanhas = []; this.unidades = []; this.eventos = [];
    if (this.estudoId) {
      this.campanhaService.listar(+this.estudoId).subscribe(c => this.campanhas = c);
    }
  }

  onCampanhaChange() {
    this.unidadeId = ''; this.eventoId = '';
    this.unidades = []; this.eventos = [];
    if (this.estudoId && this.campanhaId) {
      this.unidadeService.listar(+this.estudoId, +this.campanhaId).subscribe(u => this.unidades = u);
    }
  }

  onUnidadeChange() {
    this.eventoId = ''; this.eventos = [];
    if (this.estudoId && this.campanhaId && this.unidadeId) {
      this.eventoService.listar(+this.estudoId, +this.campanhaId, +this.unidadeId).subscribe(ev => this.eventos = ev);
    }
  }

  prosseguir() {
    if (!this.estudoId || !this.campanhaId || !this.unidadeId || !this.eventoId) return;
    this.router.navigate([
      '/estudos', this.estudoId,
      'campanhas', this.campanhaId,
      'unidades', this.unidadeId,
