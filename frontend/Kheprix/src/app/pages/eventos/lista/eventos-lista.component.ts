...
export class EventosListaComponent implements OnInit {
  ...
  estudoId!: number; campanhaId!: number; unidadeId!: number; nomeUnidade = '';
  loading = true; showDetalhes = false;
  ...
  ngOnInit() {
    ...
    this.eventoService.listar(this.estudoId, this.campanhaId, this.unidadeId).subscribe({
      next: ev => { this.eventos = ev; this.loading = false; },
      error: () => this.loading = false,
    });
    this.unidadeService.buscar(this.estudoId, this.campanhaId, this.unidadeId).subscribe(u => {
  ...
  verRegistros(ev: EventoAmostragem) {
    this.router.navigate(['/estudos', this.estudoId, 'campanhas', this.campanhaId, 'unidades', this.unidadeId, 'eventos', ev.id, 'registros']);
  }

  novoEvento() {
    this.router.navigate(['/estudos', this.estudoId, 'campanhas', this.campanhaId, 'unidades', this.unidadeId, 'eventos', 'novo']);
  }

  editarUnidade() {
    this.router.navigate(['/estudos', this.estudoId, 'campanhas', this.campanhaId, 'unidades', this.unidadeId, 'editar']);
  }
  ...
  deletar(ev: EventoAmostragem) {
    ...
    this.eventoService.deletar(this.estudoId, this.campanhaId, this.unidadeId, ev.id).subscribe({
