...
export class EspecieNovoComponent implements OnInit {
  ...
  statusList = Object.entries(StatusConservacaoLabels).map(([value, label]) => ({ value, label }));
  ...
  ngOnInit() {
    ...
    this.especieId = this.route.snapshot.params['especie_id'] ? +this.route.snapshot.params['especie_id'] : null;
    this.isEdit = !!this.especieId;
    if (this.isEdit && this.especieId) {
      this.especieService.buscar(this.estudoId, this.especieId).subscribe(e => {
        this.classe = e.classe; this.ordem = e.ordem; this.familia = e.familia;
        this.genero = e.genero; this.especie = e.especie; this.nomePopular = e.nome_popular;
        this.statusConservacao = e.status_conservacao; this.endemismo = e.endemismo;
        if (e.foto) this.fotoPreview = this.util.buildFotoUrl(this.apiUrl, e.foto);
      });
    }
  }
  ...
  salvar() {
    if (!this.classe || !this.genero || !this.especie) { this.erro = 'Preencha Classe, Gênero e Espécie.'; return; }
    this.loading = true; this.erro = '';
    const base = { classe: this.classe, ordem: this.ordem, familia: this.familia, genero: this.genero, especie: this.especie, nome_popular: this.nomePopular, status_conservacao: this.statusConservacao, endemismo: this.endemismo, ...(this.fotoBase64 ? { foto: this.fotoBase64 } : {}) };
    const obs = this.isEdit && this.especieId
      ? this.especieService.atualizar(this.estudoId, this.especieId, base)
      : this.especieService.criar(this.estudoId, base);
    obs.subscribe({
      next: (e) => this.router.navigate(['/estudos', this.estudoId, 'especies', e.id]),
