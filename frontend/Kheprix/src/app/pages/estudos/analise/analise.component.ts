...
export class AnalisesComponent implements OnInit {
  ...
  readonly categorias: CategoriaAnalise[] = [
    'modelo_distribuicao', 'rarefacao', 'estimador_riqueza',
    'indice_diversidade', 'teste_hipotese', 'correlacao',
    'regressao', 'similaridade', 'multivariada', 'glm', 'acumulacao',
  ];
  ...
  get tipoDado(): string { return this.analiseAtual?.tipo_dado ?? ''; }

  get usaAmostra(): boolean {
    return TIPOS_COM_AMOSTRA.has(this.tipoDado);
  }

  /** Tipos que dependem de vários locais distintos e portanto o escopo por campanha é relevante */
  get mostraFiltroCampanha(): boolean {
    return this.usaAmostra || this.tipoDado === 'abundancias';
  }

  /** Bounding box só faz sentido para tipos sem unidade amostral, ou fontes derivadas */
      ...
      if ((campanhas as CampanhaItem[]).length) {
        forkJoin(
          (campanhas as CampanhaItem[]).map(c =>
            this.unidadeService.listar(this.estudoId, c.id).pipe(catchError(() => of([])))
          )
        ).subscribe(grupos => {
          this.unidades = grupos.flatMap((lista, i) =>
            (lista as any[]).map(u => ({ ...u, campanha_id: (campanhas as CampanhaItem[])[i].id }))
          );
  ...
  nomeCategoria(cat: CategoriaAnalise): string { return NOME_CATEGORIA[cat]; }
  ...
  onAnaliseChange(): void {
    this.analiseAtual = this.catalogo.find(a => a.chave === this.chaveEscolhida) ?? null;
  ...
  private resetFormulario(): void {
    this.fonteX = 'variavel'; this.fonteY = 'variavel';
    this.variavelXId = undefined; this.variavelYId = undefined;
    this.fonte = 'variavel'; this.variavelId = undefined;
    ...
    this.latMin = undefined; this.latMax = undefined;
    this.lngMin = undefined; this.lngMax = undefined;
    this.resultado = null;
    this.erro = null;
  }
  ...
  toggleGrupo(grupo: 1 | 2, id: number, ev: Event): void {
    const checked = (ev.target as HTMLInputElement).checked;
    if (grupo === 1) {
      this.grupo1Ids = checked ? [...this.grupo1Ids, id] : this.grupo1Ids.filter(v => v !== id);
    } else {
      this.grupo2Ids = checked ? [...this.grupo2Ids, id] : this.grupo2Ids.filter(v => v !== id);
    }
  }
  ...
  onCoordInput(campo: 'latMin' | 'latMax' | 'lngMin' | 'lngMax', raw: string): void {
