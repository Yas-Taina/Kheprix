...
export class EstudosListaComponent implements OnInit {
  ...
  buscar() {
    ...
    if (this.tipoBusca === "nome" && this.termoBusca)
      f["nome"] = this.termoBusca;
    if (this.filtros["criado_a_partir_de"])
      f["criado_a_partir_de"] = this.filtros["criado_a_partir_de"];
    if (this.filtros["criado_ate"])
      f["criado_ate"] = this.filtros["criado_ate"];
    if (this.filtros["atualizado_a_partir_de"])
      f["atualizado_a_partir_de"] = this.filtros["atualizado_a_partir_de"];
    if (this.filtros["atualizado_ate"])
      f["atualizado_ate"] = this.filtros["atualizado_ate"];
    this.filtros = f;
    this.carregar();
  }
  ...
  verDetalhe(e: Estudo) {
    this.router.navigate(["/estudos", e.id]);
  }
  verColaboradores(e: Estudo) {
    this.router.navigate(["/estudos", e.id, "colaboradores"]);
  }
