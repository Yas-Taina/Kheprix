...
export class ColaboradoresComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit() {
    ...
    this.colabService.listar(this.estudoId).subscribe(c => this.colaboradores = c);
    this.codigoService.buscar(this.estudoId).subscribe({
      next: (c) => { this.codigoAcesso = c; this.loadingCodigo = false; setTimeout(() => this.gerarQrCode(), 100); },
  ...
  convidar() {
    ...
    this.loadingConvite = true; this.erroConvite = '';
    this.conviteService.criar(this.estudoId, this.emailConvite).subscribe({
      next: () => { this.msgConvite = 'Convite enviado!'; this.emailConvite = ''; this.loadingConvite = false; },
      error: () => { this.erroConvite = 'Erro ao enviar convite.'; this.loadingConvite = false; },
    });
  }

  alterarAcesso(c: Colaborador) {
    this.colabService.atualizar(this.estudoId, c.id_usuario, { perfil: c.perfil }).subscribe();
  }

  remover(c: Colaborador) {
    if (!confirm(`Remover "${c.nome}"?`)) return;
    this.colabService.deletar(this.estudoId, c.id_usuario).subscribe({
      next: () => this.colaboradores = this.colaboradores.filter(x => x.id_usuario !== c.id_usuario),
    });
  }
  ...
  alterarSenha() {
    ...
    this.codigoService.atualizar(this.estudoId, this.novaSenhaAcesso).subscribe({
      next: (c) => { this.codigoAcesso = c; this.novaSenhaAcesso = ''; this.loadingSenha = false; },
