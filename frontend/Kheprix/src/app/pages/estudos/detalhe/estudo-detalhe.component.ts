...
import { EstudoDashboard, AgrupamentoTempo } from '../../../models/dashboard.model';
...
export class EstudoDetalheComponent implements OnInit, OnDestroy {
  ...
  exportar(): void {
    ...
    this.exportMsg = '';
    this.estudoService.exportarDados(this.estudoId, this.agrupamento, this.formato).subscribe({
      next: (blob) => {
  ...
  carregarDashboard(): void {
    ...
    this.dashboardService.getDashboard(this.estudoId).subscribe({
      ...
      error: () => {
        this.dashboardError.set('Não foi possível carregar os dados do dashboard.');
        this.dashboardLoading.set(false);
      },
    });
  }
  ...
  private buildRegistrosData(d: EstudoDashboard): void {
    this.chartRegistrosData?.destroy();
    const ctx = document.getElementById('chart-registros-data') as HTMLCanvasElement;
    if (!ctx) return;
    this.chartRegistrosData = new Chart(ctx, {
      ...
      data: {
        ...
        datasets: [{
          ...
          pointRadius: 3, pointBackgroundColor: '#6B7D5E',
        }],
      },
  ...
  private buildOcorrencias(d: EstudoDashboard): void {
    this.chartOcorrencias?.destroy();
    const ctx = document.getElementById('chart-ocorrencias') as HTMLCanvasElement;
    if (!ctx) return;
    const cores = ['#4A4035','#6B7D5E','#8FA082','#A8B99C','#C5BA9F','#B3C4A7','#DDD8C4'];
    this.chartOcorrencias = new Chart(ctx, {
      type: 'doughnut',
      data: {
        labels: d.ocorrencias_por_especie.map(e => e.nome_popular || e.nome_cientifico),
        datasets: [{ data: d.ocorrencias_por_especie.map(e => e.total), backgroundColor: cores, borderWidth: 0 }],
      },
      options: {
        responsive: true, maintainAspectRatio: false, cutout: '60%',
        plugins: {
          legend: { position: 'right', labels: { font: this.fontCormorant(12), color: '#4A4035', boxWidth: 13 } },
        },
      },
    });
  }
  ...
  buildEspecieTempo(d?: EstudoDashboard): void {
    ...
    if (!data) return;
    this.chartEspecieTempo?.destroy();
    const ctx = document.getElementById('chart-especie-tempo') as HTMLCanvasElement;
    if (!ctx) return;

    const especie = this.selectedEspecie();
    const filtrado = especie
      ? data.registros_por_especie_tempo.filter(r => r.nome_cientifico === especie)
    ...
    this.chartEspecieTempo = new Chart(ctx, {
      ...
      data: {
        ...
        datasets: [{
          ...
          borderColor: '#6B7D5E', backgroundColor: 'rgba(107,125,94,0.10)',
          tension: 0.4, fill: true,
          pointRadius: 3, pointBackgroundColor: '#6B7D5E',
        }],
      },
  ...
  private buildNativasInvasoras(d: EstudoDashboard): void {
    this.chartNativasInvasoras?.destroy();
    const ctx = document.getElementById('chart-nativas-invasoras') as HTMLCanvasElement;
    if (!ctx) return;
    this.chartNativasInvasoras = new Chart(ctx, {
      ...
      data: {
        labels: [''],
        datasets: [
          { label: 'Espécies Nativas',   data: [d.resumo.especies_nativas],   backgroundColor: '#4A4035', borderRadius: 3 },
          { label: 'Espécies Invasoras', data: [d.resumo.especies_invasoras], backgroundColor: '#8FA082', borderRadius: 3 },
        ],
      },
      options: {
        indexAxis: 'y', responsive: true, maintainAspectRatio: false,
        plugins: { legend: { position: 'top', labels: { font: this.fontCormorant(12), color: '#4A4035', boxWidth: 12 } } },
        scales: {
          x: { grid: { color: 'rgba(0,0,0,0.05)' }, ticks: { font: this.fontCormorant(), color: '#6B5F52' } },
          y: { display: false },
        },
      },
    });
  ...
  buildEspeciesDistintas(d?: EstudoDashboard): void {
    ...
    if (!data) return;
    this.chartEspeciesDistintas?.destroy();
    const ctx = document.getElementById('chart-especies-distintas') as HTMLCanvasElement;
    if (!ctx) return;
    ...
    this.chartEspeciesDistintas = new Chart(ctx, {
      ...
      data: {
        labels,
        datasets: [{ data: valores, backgroundColor: labels.map((_,i) => paleta[i % paleta.length]), borderRadius: 3 }],
      },
      options: {
        indexAxis: 'y', responsive: true, maintainAspectRatio: false,
  ...
  onEspecieChange(): void     { this.buildEspecieTempo(); }
  ...
  buildEspeciesDistintasTempo(d?: EstudoDashboard): void {
    ...
    if (!data) return;
    this.chartEspeciesDistintasTempo?.destroy();
    const ctx = document.getElementById('chart-especies-distintas-tempo') as HTMLCanvasElement;
    if (!ctx) return;
    ...
    if (this.selectedAgrupamento() === 'ano') {
      ...
      labels  = ordenado.map(r => `${String(r.mes).padStart(2,'0')}/${r.ano}`);
  ...
  private opcoesLinha(yLabel: string, xLabel: string): any {
    return {
      ...
      scales: {
        y: {
          title: { display: true, text: yLabel, font: this.fontCormorant(11), color: '#6B5F52' },
          grid: { color: 'rgba(0,0,0,0.05)' },
          ticks: { font: this.fontCormorant(), color: '#6B5F52' },
        },
        x: {
          title: { display: true, text: xLabel, font: this.fontCormorant(11), color: '#6B5F52' },
          grid: { display: false },
          ticks: { font: this.fontCormorant(10), color: '#6B5F52', maxRotation: 45 },
        },
      },
    };
  }
  ...
  onEspecieChange(): void    { this.buildEspecieTempo(); }
  onAgrupamentoChange(): void { this.buildEspeciesDistintas(); }
