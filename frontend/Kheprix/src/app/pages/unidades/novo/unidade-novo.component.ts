...
export class UnidadeNovoComponent implements OnInit {
  estudoId!: number; campanhaId!: number; unidadeId: number | null = null; isEdit = false;
  nome = ''; latDMS = ''; lngDMS = ''; raio: number | null = null;
  metodoColeta = ''; esforcoAmostral = '';
  ...
  ngOnInit() {
    ...
    this.unidadeId = this.route.snapshot.params['unidade_id'] ? +this.route.snapshot.params['unidade_id'] : null;
  ...
  async obterLocalizacao() {
    ...
    try {
      ...
      this.gpsErro = 'Não foi possível obter a localização. Você pode preencher manualmente.';
    } finally {
  ...
  salvar() {
    if (!this.nome || !this.latDMS || !this.lngDMS) { this.erro = 'Nome e coordenadas são obrigatórios.'; return; }
    this.loading = true; this.erro = '';
    const lat = this.util.dmsTodecimal(this.latDMS);
    const lng = this.util.dmsTodecimal(this.lngDMS);
    const payload = {
      nome: this.nome, latitude: lat, longitude: lng,
