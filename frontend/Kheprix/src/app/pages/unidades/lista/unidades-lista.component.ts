...
export class UnidadesListaComponent implements OnInit {
  ...
  estudoId!: number; campanhaId!: number; nomeCampanha = '';
  ...
  formatCoord(dec: number): string { return this.util.decimalToDMS(dec); }
  ...
  editarCampanha() {
    this.router.navigate(['/estudos', this.estudoId, 'campanhas', this.campanhaId, 'editar']);
  }

  deletar(u: UnidadeAmostral) {
    if (!confirm(`Excluir "${u.nome}"?`)) return;
    this.unidadeService.deletar(this.estudoId, this.campanhaId, u.id).subscribe({
      next: () => this.unidades = this.unidades.filter(x => x.id !== u.id),
    });
  }
}
